...
def safe_input(string="",expect_type=str,filter=lambda x: x):
    while True:
        if not(isinstance(expect_type,re.Pattern)): # если не регулярное выражение
            try:
                temp = expect_type(input(string)) # Если expect_type — скомпилированное регулярное выражение, то проверяем ввод через полное совпадение (re.fullmatch)
...
def closest_match(target,choices):
    return difflib.get_close_matches(target,choices,n=1,cutoff=0)[0] # cutoff=0 гарантирует, что всегда вернётся хотя бы один "наиболее близкий" вариант,
...
class Card:
    def __init__(self,name):
        self.id = "".join([str(randint(0,9)) for i in range(16)])# Учебная реализация: настоящие номера карт должны проходить проверку по алгоритму Луна

        self.expiration = (datetime.now().day,datetime.now().year+10)
        self.code = "".join([str(randint(0,9)) for i in range(3)]) # В реальных системах CVV НИКОГДА не хранится и не проверяется таким образом!
...
class Bank_Account:
    ...
    def create_card(self):
        ...
        confirmation = closest_match(safe_input("Вы уверены что хотите создать новую банковскую карту? ").lower(),["да","нет"])
        if confirmation=="да":
    ...
    def add_funds(self):
        ...
        amount = safe_input(f"Введите сколько хотите пополнить для карты {choiced_card.name} ",int)
        confirmation=closest_match(safe_input(f"Делаем пополнение для карты {choiced_card.name} в размере {amount}? "),["да","нет"])
        if confirmation=="да":
    ...
    def deduction(self):
        ...
        choiced_card = self.find_card_by_id(_choice)
        choiced_card.statistic()
        amount = safe_input(f"Введите сколько хотите снять с карты {choiced_card.name} ",int,lambda x:x<=choiced_card.balance)
        confirmation=closest_match(safe_input(f"Списываем с карты {choiced_card.name} {amount}? "),["да","нет"])
        if confirmation=="да":
            choiced_card.balance-=amount
            choiced_card.statistic(id=0,ccv=0,exp=0)
    def transfer(self):
        while True:
            card_id = safe_input("Введите номер вашей карты, с которой будет производится списание:\n ", re.compile(r"^\d{13,19}$"))
            ...
            if card_CCV==card.code:
                card.statistic()
                money = safe_input("Введите сумму, которую хотите списать ", int, filter=lambda x: x <= card.balance)
                card_id2 = safe_input("Введите номер вашей карты, на которую будет производится пополнение:\n ",
                                          re.compile(r"^\d{13,19}$"))
                ...
                if card_CCV2==card2.code:
                    ...
                    card2.statistic()
                    inp = safe_input(f"Подтвердить перевод c карты {card.name} на карту {card2.name} в размере {money}? ")
    ...
    def GUI(self):
        print("Добро пожаловать в меню управления счётом в банке! Чтобы выйти напишите 'выход'")

        while True:
            print("Выберите действие:")
            moves= ["перевод","пополнение","списание","проверка",
                    "создание"]

            function_call = dict(zip(moves,[self.transfer,self.add_funds,self.deduction,self.check_bank_account,self.create_card])) # вызов соответствующей функции
            move_raw=safe_input("Перевод/Пополнение счёта / Списание со счёта / Проверка баланса счёта / Создание новой карты ")
            if move_raw=="выход":
                return 1
            function_call[closest_match(move_raw.lower(),["перевод","пополнение","списание","проверка","создание"])]() # преобразование ввода пользователя и вызов нужной функции
